<post>
    <li class="media post post-{opts.post.id} list-group-item p-4 my-3">
      <img class="media-object mr-3 align-self-start" src="{ opts.post.user_url }">
      <div class="media-body">
        <div class="media-heading">
          <small class="float-right text-muted">{opts.post.days_since} days</small>
          <h6><a href="{root_url}/users/{opts.post.user}">{opts.post.user_name}</a> made the commitment: <a href="{root_url}/posts/{opts.post.id}">{opts.post.title}</a></h6>
        </div>
        <p>
    ...
    <div if={opts.post.proof_description && opts.post.proof_pic} id="accordion{opts.post.id}" role="tablist">
      <div class="card">
        <div class="card-header" role="tab" id="heading{opts.post.id}">
          <h5 class="mb-0">
            <a data-toggle="collapse" href="#collapse{opts.post.id}" aria-expanded="true" aria-controls="collapse{opts.post.id}">
              {opts.post.user_name} submitted proof, it took {opts.post.days_taken} days.
            </a>
          </h5>
        </div>
        <div id="collapse{opts.post.id}" class="collapse show" role="tabpanel" aria-labelledby="heading{opts.post.id}" data-parent="#accordion{opts.post.id}">
          <div class="card-body">
            <div class="post-proof">
              <p>{opts.post.proof_description}</p>
